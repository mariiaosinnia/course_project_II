...
std::vector<uint8_t> MusicStreamer::buildPacket(uint32_t seq, const std::vector<uint8_t>& opus, uint32_t track_position_ms) {
    std::vector<uint8_t> packet(9 + opus.size());   // 1 (type) + 4 (seq) + 4 (position) + opus
...
std::vector<uint8_t> MusicStreamer::buildVoicePacket(uint32_t sender_client_id, uint32_t seq, const std::vector<uint8_t>& opus) {
