...
UdpAudioServer::UdpAudioServer(unsigned short udp_port,
...
UdpAudioServer::prepareTrack(const std::string &file_path) {
  ...
  for (size_t i = 0; i < total_frames; ++i) {
    ...
    track->opus_packets.push_back(
        encoder.encodeFrame(frame_start, samples_per_frame));
  }

  track->pcm_data.clear();
  track->pcm_data.shrink_to_fit();

  std::cout << "Track loaded: " << file_path << " | rate=" << track->sample_rate
        ...
        if (now >= state.next_send_time) {
          ...
          if (state.current_packet_index >= state.track->opus_packets.size()) {
            ...
            uint16_t next_track_id =
                room_manager_.advance_track(it->first, now);
            ...
            if (next_it != tracks_.end()) {
              ...
              std::cout << "Room " << it->first << " switched to track "
                        << next_track_id << "\n";
            } else {
              std::cerr << "Error: Next track " << next_track_id
                        << " not loaded.\n";
              it = active_rooms_.erase(it);
              continue;
            }
          ...
          const auto &opus_data =
              state.track->opus_packets[state.current_packet_index];
          // додаю час треку всередині пакету для drift correction
          uint32_t track_position_ms =
              state.current_packet_index * FRAME_DURATION_MS;
          auto packet = MusicStreamer::buildPacket(
              state.sequence_number, opus_data, track_position_ms);
...
void UdpAudioServer::startStreaming(uint16_t room_id) {
  ...
  std::cout << "Streaming started for room " << room_id << " with track "
            << track_id << "\n";
}

void UdpAudioServer::registerClient(uint32_t client_id,
...
void UdpAudioServer::receiveLoop() {
  ...
  while (running) {
    ...
    case UdpPacketType::Registration: {
      if (bytes == 5) {
        uint32_t client_id = (static_cast<uint32_t>(buffer[1]) << 24) |
    ...
    case UdpPacketType::Voice: {
      if (auto route_opt = handleVoicePacket(buffer, bytes, sender)) {
        for (const auto &endpoint : route_opt->endpoints) {
          udp_socket_.sendTo(route_opt->packet, endpoint);
        }
      }
      break;
    }
...
UdpAudioServer::handleVoicePacket(const std::vector<uint8_t> &buffer,
                                  size_t bytes, const udp::endpoint &sender) {
  if (bytes < 9)
    return std::nullopt;

  uint32_t sender_client_id = (static_cast<uint32_t>(buffer[1]) << 24) |
                              (static_cast<uint32_t>(buffer[2]) << 16) |
                              (static_cast<uint32_t>(buffer[3]) << 8) |
                              (static_cast<uint32_t>(buffer[4]));
  ...
  VoiceRoutingResult result;
  result.packet.assign(buffer.begin(), buffer.begin() + bytes);
  result.endpoints =
      room_manager_.get_udp_endpoints_except(room_id, sender_client_id);
  return result;
}
